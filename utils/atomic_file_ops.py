        ...
        while True:
            try:
                ...
                if time.time() - start_time > timeout:
                    logger.error(f"L6.atomic [lock] - Timeout waiting for file lock: {file_path}")
    ...
    def load_json_safe(file_path: Path, default: Optional[Dict] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if not file_path.exists():
                logger.debug(f"L6.atomic [load] - File doesn't exist, returning default: {file_path}")
                return default.copy()
            
            # Load with file locking
            with AtomicFileOperations.file_lock(file_path, 'r') as f:
                data = json.load(f)
                logger.debug(f"L6.atomic [load] - Successfully loaded JSON: {file_path}")
                return data
                
        except json.JSONDecodeError as e:
            logger.warning(f"L6.atomic [load] - JSON decode error, returning default: {file_path} - {e}")
            return default.copy()
        except TimeoutError as e:
            logger.error(f"L6.atomic [load] - Lock timeout: {e}")
            return default.copy()
        except Exception as e:
            logger.error(f"L6.atomic [load] - Unexpected error loading {file_path}: {e}")
            return default.copy()
        ...
        try:
            ...
            try:
                with open(lock_file, 'w') as lock_fd:
                    ...
                    logger.debug(f"L6.atomic [save] - Successfully saved JSON atomically: {file_path}")
            ...
            logger.error(f"L6.atomic [save] - Failed to save {file_path}: {e}")
