# xAI (Grok) Prompt Examples - COMPLETE & UNTRUNCATED

This document shows the **exact, complete, verbatim** prompts that Theo sends to xAI's models.

## Overview

xAI uses a messages array with proper role separation:
- `system`: AI identity, capabilities, and context  
- `user`: The actual user query

Theo's 3-message structure:
1. **Message 1 (system)**: System instructions + Available tools
2. **Message 2 (system)**: Context (Memory Bank, Working Memory, Chat History)
...
- **Native Features**: Live web search (enabled by default)
...
- **Proper Role Labeling**: System context is labeled as `system`, only the actual user query is labeled as `user`
- **Message 1**: System instructions and tools (AI's role and capabilities)
- **Message 2**: Context including memories, working memory, and chat history (labeled as `system` for clarity)
- **Message 3**: The actual user message (only this is labeled `user`)
- For new rooms, memory bank and chat history are empty
- This structure follows best practices for clear delimiters between instructions, context, and user input
...
- `layer1_chatbot/model_call_xai.py` - xAI API integration
